...
class TestNamespaceTranslator(unittest.TestCase):
    ...
    def test_is_migrated_ignores_intent_topic_pairs(self):
        """is_migrated is the NARROW namespace check: intent pairs are not in it."""
        t = NamespaceTranslator()
        self.assertFalse(t.is_migrated("skill.foo:food.order"))
        self.assertFalse(t.is_migrated("skill.foo:food.order.intent"))

    def test_has_mirror_covers_both_bridges(self):
        t = NamespaceTranslator()
        # namespace bridge (static map, both directions)
        self.assertTrue(t.has_mirror("speak"))
        self.assertTrue(t.has_mirror("ovos.utterance.speak"))
        # namespace bridge (computed <skill_id>:stop dispatch)
        self.assertTrue(t.has_mirror("lights.skill:stop"))
        # intent-topic bridge (runtime pairing, both spellings)
        self.assertTrue(t.has_mirror("skill.foo:food.order"))
        self.assertTrue(t.has_mirror("skill.foo:food.order.intent"))

    def test_has_mirror_rejects_unrelated_topics(self):
        t = NamespaceTranslator()
        self.assertFalse(t.has_mirror("random.topic"))
        self.assertFalse(t.has_mirror("ovos.some.unmapped.topic"))
        # a reserved namespace is not a per-intent dispatch topic, so the
        # ``.intent`` suffix does not make it one half of an intent pair
        self.assertFalse(t.has_mirror("recognizer_loop:utterance.intent"))

    def test_has_mirror_agrees_with_mirror_counterpart(self):
        t = NamespaceTranslator()
        for topic in ("speak", "ovos.utterance.speak", "lights.skill:stop",
                      "skill.foo:food.order", "skill.foo:food.order.intent",
                      "random.topic", "recognizer_loop:utterance",
                      "recognizer_loop:utterance.intent"):
            self.assertEqual(t.has_mirror(topic),
                             mirror_counterpart(topic) is not None, topic)
