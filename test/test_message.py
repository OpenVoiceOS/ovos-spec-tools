...
class TestReply:
    ...
    def test_reply_strips_none_members_from_list_source(self):
        """A ``None`` member of the array-of-strings form is an identifier
        under §3.3 too, so it is dropped the same as a scalar ``None``
        would be, and never picked as the new ``source``."""
        m = Message("q.ask", {}, {"source": "A",
                                   "destination": [None, "B"]})
        r = m.reply("q.ans")
        assert r.context.get("source") == "B"
        assert r.context.get("source") is not None
    ...
    def test_reply_empty_routing_key_equals_absent_key(self, source,
        ...
        def routing(ctx):
            return {k: ctx[k] for k in ("source", "destination") if k in ctx}

        got = routing(build(drop_empty=False))
        assert got == routing(build(drop_empty=True))
