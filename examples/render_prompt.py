...
A `.prompt` is whole-file plain text: substitution is conservative — literal
braces, unfilled slots, and fenced code blocks are all left untouched.
...
# and `{query}` are supplied — the literal JSON braces and the `{example}`
# inside the fenced code block are left exactly as written.
